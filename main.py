...
# dm_log is the top of the hierarchy, root is not used
# 256K max file size, 4 files max
log_format    = '%(asctime)s ' + '%(levelname)s %(message)s'
log_datefmt   = '%m/%d/%Y %H:%M:%S '
log_formatter = logging.Formatter(fmt=log_format, datefmt=log_datefmt)
dm_log        = logging.getLogger('rl.dm')
dm_log_fh     = logging.handlers.RotatingFileHandler(DM_LOG_FILE, maxBytes=(256*1024), backupCount=3)
dm_log_fh.setLevel('DEBUG')
dm_log_fh.setFormatter(log_formatter)
dm_log.addHandler(dm_log_fh)

dm_log.info("")
dm_log.info("MODULE STARTING...")
...
import boot.py     # start serial stream driver running on core 1
